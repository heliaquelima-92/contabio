...
export default function Header({ desktop = false }) {
  ...
  if (desktop) {
    return (
      <header className="bg-moncash-dark border-b border-moncash-border px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs text-moncash-text-muted">{obterSaudacao()}</p>
            <h1 className="text-xl font-bold text-white">Bem-vindo, {nomeUsuario}!</h1>
          </div>

          <div className="relative">
            <button
              onClick={() => { setMenuAberto(!menuAberto); som.tocar('clique') }}
              className="flex items-center gap-3 px-4 py-2 rounded-xl bg-moncash-card hover:bg-moncash-card-hover transition-colors"
            >
              <div className="text-right">
                <p className="text-sm font-medium text-white">{nomeUsuario}</p>
                <p className="text-xs text-moncash-text-muted">{usuario?.email}</p>
              </div>
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-moncash-lime to-moncash-lime-dark
  ...
  return (
    <header className="sticky top-0 z-40 bg-moncash-dark/90 backdrop-blur-xl safe-top">
      <div className="px-4 py-3 flex items-center justify-between">
        ...
        >
          <img src="https://i.imgur.com/igiIEnb.png" alt="Moncash" className="h-7" />
        </motion.div>
