...
export function DadosProvider({ children }) {
  ...
  const deletarContaFixa = async (id) => {
    ...
    // Também remover do mês atual
    setContasMes(prev => prev.filter(c => c.conta_fixa_id !== id))
  }
  ...
  const deletarCartao = async (id) => {
    ...
    setContasMes(prev => prev.filter(c => c.cartao_id !== id))
  }
  ...
  const atualizarMetaCofrinho = async (meta) => {
    const atualizado = await db.salvarCofrinho({ usuario_id: usuario.id, total: cofrinho?.total || 0, meta })
  ...
  // Gerar contas do mês (para meses futuros/anteriores)
  const gerarContasMes = async () => {
    ...
    for (const fixa of contasFixas) {
      novasContas.push({
        usuario_id: usuario.id, mes: mesAtual, ano: anoAtual,
  ...
  const value = {
    ...
    adicionarGasto, deletarGasto, adicionarAoCofrinho, atualizarMetaCofrinho,
