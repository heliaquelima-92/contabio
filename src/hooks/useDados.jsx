...
export function DadosProvider({ children }) {
  ...
  const carregarContasAtrasadas = useCallback(async () => {
    if (!usuario) return []
    try {
      const { mes, ano } = obterMesAnoAtual()
      const data = await db.listarContasAtrasadas(usuario.id, mes, ano)
      setContasAtrasadas(data || [])
      return data || []
    } catch (error) {
  ...
  const adicionarContaParcelada = async (conta) => {
    // Guardar mês e ano de início para calcular parcelas automaticamente
    const nova = await db.criarContaParcelada({ 
      ...
      mes_inicio: mesAtual,
      ano_inicio: anoAtual,
      parcelas_restantes: conta.parcelas_totais // Começa com todas as parcelas
    })
    setContasParceladas(prev => [...prev, nova])
    
    // Criar primeira parcela (1/X) no mês atual
    const contaMes = await db.criarContaMes({
      usuario_id: usuario.id,
      mes: mesAtual,
      ano: anoAtual,
      nome: nova.nome + ' (1/' + nova.parcelas_totais + ')',
  ...
  const gerarContasParaMes = async (mes, ano) => {
    ...
    // Buscar contas já existentes nesse mês para evitar duplicatas
    const contasExistentes = await db.listarContasMes(usuario.id, mes, ano)
    const fixaIdsExistentes = (contasExistentes || []).filter(c => c.conta_fixa_id).map(c => c.conta_fixa_id)
    const parceladaIdsExistentes = (contasExistentes || []).filter(c => c.conta_parcelada_id).map(c => c.conta_parcelada_id)
    const cartaoIdsExistentes = (contasExistentes || []).filter(c => c.cartao_id).map(c => c.cartao_id)
    
    const novasContas = []
    
    // Contas fixas - apenas as ATIVAS e que não existem ainda
    for (const fixa of (contasFixasAtuais || [])) {
      if (fixa.ativa !== false && !fixaIdsExistentes.includes(fixa.id)) {
        novasContas.push({
    ...
    // Parceladas - calcular qual parcela é baseado no mês/ano de início
    for (const parcelada of (contasParceladasAtuais || [])) {
      const naoArquivada = !parcelada.arquivada
      const naoExiste = !parceladaIdsExistentes.includes(parcelada.id)
      
      // Calcular qual parcela é baseado na diferença de meses
      const mesInicio = parcelada.mes_inicio || mes
      const anoInicio = parcelada.ano_inicio || ano
      const mesesDesdeInicio = (ano - anoInicio) * 12 + (mes - mesInicio)
      const parcelaAtual = mesesDesdeInicio + 1 // Parcela 1 no mês de início
      
      // Só gera se está dentro do número de parcelas e não está arquivada
      const dentroDoLimite = parcelaAtual >= 1 && parcelaAtual <= parcelada.parcelas_totais
      
      if (naoArquivada && naoExiste && dentroDoLimite) {
        novasContas.push({
          usuario_id: usuario.id, mes, ano,
    ...
    // Cartões - apenas os ATIVOS e que não existem ainda
    for (const cartao of (cartoesAtuais || [])) {
      if (cartao.ativa !== false && !cartaoIdsExistentes.includes(cartao.id)) {
        novasContas.push({
          usuario_id: usuario.id, mes, ano,
    ...
    // Criar as contas no banco (apenas as novas)
  ...
  const value = {
    carregando, configuracoes, contasFixas, contasParceladas, cartoes, contasMes, contasAtrasadas, gastos, cofrinho, cofrinhoHistorico, mesAtual, anoAtual,
    ...
    mudarMes, gerarContasMes, carregarTudo, carregarContasAtrasadas, ...calcularTotais()
  }
